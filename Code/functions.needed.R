...
import.TC <- function(Coeff, Materials, ignore_missing = FALSE){
  
  TC <- sapply(Materials, function(x) NULL)
  fromcomp <- unique(Coeff[,"From"])
  
  # Initialize list to store ignored flows
  ignored_flows <- list()
  ...
  for(comp in fromcomp){
    ...
    for(dest in tocomp){
      ...
      for(mat in Materials){
        ...
        # If no matching material
        if(length(datapoints) == 0){
          if(ignore_missing){
            warning_msg <- paste("No matching material for", comp, "to", dest, "material", mat, "- ignored.")
            warning(warning_msg)
            ignored_flows <- append(ignored_flows,
                                    list(data.frame(From=comp, To=dest, Material=mat, Note="Ignored missing material")))
            next
          } else {
            stop(paste("No matching material for", comp, "to", dest, "material", mat))
          }
        }
        
        vals <- data$Data[datapoints]
        spreads <- data$Spread[datapoints]
        
        # Only check Spread if Data != 0,1,rest
        check_idx <- !(vals %in% c(0,1,"rest"))
        if(any(is.na(vals[check_idx])) || any(is.na(spreads[check_idx]))){
          stop(paste("Missing Data or Spread for flow from", comp, "to", dest, "material", mat))
        }
        ...
        # Create distribution
        TC[[mat]][[comp]][[dest]] <- rwrap(
          values = as.numeric(vals),
          perc = as.numeric(spreads),
          N = SIM,
          linf = 0,
          lsup = 1
        )
  ...
  # Print all ignored flows at the end
  if(ignore_missing && length(ignored_flows) > 0){
    message("==== Ignored flows due to missing material ====")
    print(do.call(rbind, ignored_flows))
  }
...
calc.rest.TC <- function(TC){
  ...
  for(mat in 1:length(TC)){
    for(comp in 1:length(TC[[mat]])){
      ...
      # in case there is no other flow
      if(length(otherflows) == 0){
        TC[[mat]][[comp]][[restcomp]] <- 1
        next
      }
...
import.input <- function(data, Materials){
  ...
  data <- data[, c("Compartment", "Material", "Data", "Spread")]
  ...
  for(comp in compartment){
    Materials <- unique(data[data[, "Compartment"] == comp, "Material"])
    # loop over materials
    for(mat in Materials){
      ...
      # create distribution
      if(all(as.numeric(data[datapoints, "Data"]) >= 0)){
        Input[[mat]][[comp]] <- rwrap(values = as.numeric(data[datapoints, "Data"]), 
                                      perc = as.numeric(data[datapoints, "Spread"]),
                                      N = SIM,
                                      linf = 0)
      } else if(all(as.numeric(data[datapoints, "Data"]) <= 0)){
        Input[[mat]][[comp]] <- rwrap(values = as.numeric(data[datapoints, "Data"]), 
                                      perc = as.numeric(data[datapoints, "Spread"]),
                                      N = SIM,
                                      lsup = 0)
